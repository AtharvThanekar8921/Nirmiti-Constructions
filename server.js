...
app.post('/api/addDailyReport', (req, res) => {
    const { vehicleName, date, startread, stopread, km, Drequired, Dread, avg, operator, rail, slipper, load, unload, purpose,supervisor } = req.body;
...
// Fetch and sort daily reports by date in ascending order
app.get('/api/dailyReports', (req, res) => {
    ...
    DailyReport.find(filter)
        ...
        .catch((err) => res.status(500).send('Error fetching reports: ' + err.message));
});


app.get('/api/filterDailyReports', (req, res) => {
    ...
    .catch((err) => {
        res.status(500).send('Error fetching filtered reports: ' + err.message);
    });
});
